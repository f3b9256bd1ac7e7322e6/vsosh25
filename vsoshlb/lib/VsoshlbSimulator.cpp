...
std::unique_ptr<folly::IOBuf> createPacketFromFlow(const VsoshlbFlow& flow) {
...
VsoshlbSimulator::VsoshlbSimulator(int progFd) : progFd_(progFd) {
  affinitizeSimulatorThread();
}

VsoshlbSimulator::~VsoshlbSimulator() {}

std::unique_ptr<folly::IOBuf> VsoshlbSimulator::runSimulation(
...
std::unique_ptr<folly::IOBuf> VsoshlbSimulator::runSimulationInternal(
    std::unique_ptr<folly::IOBuf> pckt) {
...
const std::string VsoshlbSimulator::getRealForFlow(const VsoshlbFlow& flow) {
...
void VsoshlbSimulator::affinitizeSimulatorThread() {
  simulatorEvb_.getEventBase()->runInEventBaseThreadAndWait([]() {
    cpu_set_t cpuSet;
    CPU_ZERO(&cpuSet);
    CPU_SET(0, &cpuSet);
