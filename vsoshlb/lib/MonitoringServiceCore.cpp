#include "vsoshlb/lib/MonitoringServiceCore.h"
#include <fcntl.h>
#include <fmt/core.h>
...
bool MonitoringServiceCore::initialize(std::shared_ptr<VsoshlbMonitor> monitor) {
  if (initialized_) {
    return true;
  }
  if (monitor == nullptr) {
  ...
  for (const auto& eventId : eventIds) {
    ...
    // going to be destroyed before/after vsoshlb monitor. If this handler and
    // its reader event base gets destroyed prior to vsoshlb monitor, when
    // vsoshlb monitor gets destroyed later, we don't want this AsyncPipeWriter
