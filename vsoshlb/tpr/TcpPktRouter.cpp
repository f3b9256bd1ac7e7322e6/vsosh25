...
#include <vsoshlb/tpr/TcpPktRouter.h>
#include <vsoshlb/tpr/bpf_util/BpfUtil.h>
...
folly::Expected<folly::Unit, std::system_error> TcpPktRouter::init(
  ...
  res = skel_.open();
  if (!res) {
    LOG(ERROR) << "Failed to open Vsoshlb TPR prog: " << res.error().what();
    return makeError(res.error(), __func__);
  }
  ...
  res = skel_.load();
  if (!res) {
    LOG(ERROR) << "Failed to load Vsoshlb TPR prog: " << res.error().what();
    return makeError(res.error(), __func__);
  }
